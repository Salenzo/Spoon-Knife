...
def get_link() -> str:
    subscribe = session.get("https://scc.ustc.edu.rs/api/v1/user/getSubscribe")
    return subscribe.json()["data"]["subscribe_url"]


def change_link() -> str:
    reset = session.get("https://scc.ustc.edu.rs/api/v1/user/resetSecurity")
...
login = session.post("https://scc.ustc.edu.rs/api/v1/passport/auth/login", data={
    "email": user[0],
    "password": user[1]
})
